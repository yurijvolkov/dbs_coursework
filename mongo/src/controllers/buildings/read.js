...
export async function readAll(req, res) {
  try {
    const {query: {limit = 0, offset = 0}} = req;
    const resultSet = await Buildings.find({}).skip(Number.parseInt(offset)).limit(Number.parseInt(limit)).exec();
...
export async function readById(req, res) {
  try {
    ...
    const result = await Buildings
      .findById(buildingId)
      .populate('streets', '_id graphId name')
      .exec();
...
export async function readBy(req, res) {
  ...
  try {
    const resultSet = await Buildings.find(query).exec();

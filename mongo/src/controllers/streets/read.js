...
export async function readAll(req, res) {
  try {
    const resultSet = await Streets.find({}).exec();
...
export async function readById(req, res) {
  try {
    ...
    const result = await Streets
      .findById(streetId)
      .populate('buildings', 'address _id graphId name type')
      .exec();
...
export async function readBy(req, res) {
  ...
  try {
    const resultSet = await Streets.find(query).exec();
